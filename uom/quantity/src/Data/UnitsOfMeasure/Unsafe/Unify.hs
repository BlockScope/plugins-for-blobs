...
substsUnitEquality s (UnitEquality ct u v) =
    UnitEquality ct (substsUnit u s) (substsUnit v s)
...
unifyOne uds ct tvs subst unsubst u
    ...
    where
        ...
        go ls (at@(VarAtom a, i) : xs) = do
            ...
            if | tch && divisible i u -> return $
                    if occurs a r
                    then Draw tvs subst unsubst
                    else Win tvs (extendSubst (SubstItem a r ct) subst) unsubst

               | tch && not (all (isBase . fst) xs) -> do
                    beta <- newUnitVar
                    let subst' = extendSubst (SubstItem a (varUnit beta *: r) ct) subst
                    let unsubst' = extendSubst (SubstItem beta (varUnit a    /: r) ct) unsubst
        ...
        go ls (at@(FamAtom f tys, i) : xs) = do
            ...
            case normaliseUnit uds . snd =<< mb of
                Just v ->
                    unifyOne uds ct tvs subst unsubst $
                        mkNormUnit (ls ++ xs) *: v ^: i
        ...
        newUnitVar
            | given_mode = newSkolemTyVar $ unitKind uds
...
instance Outputable SimplifyState where
    ppr ss =
        text "fresh = " <+> ppr (simplifyFreshVars ss)
...
simplifyUnits uds eqs0 =
    tcPluginTrace "simplifyUnits" (ppr eqs0) >> simples initialState eqs0
    where
        ...
        simples ss (eq:eqs) = do
            ...
            case ur of
                Win tvs subst unsubst ->
                    let (ss', xs) = win eq tvs subst unsubst ss
                    in simples ss' (xs ++ eqs)

                Draw _ [] _ -> simples (addStuck eq ss) eqs

                Draw tvs subst unsubst ->
                    let (ss', xs) = draw eq tvs subst unsubst ss
                    in simples ss' (xs ++ eqs)

                Lose ->
                    return
                        Impossible
                            { simplifyImpossible = eq
...
win eq tvs subst unsubst ss =
  ( SimplifyState
      { simplifyFreshVars = simplifyFreshVars ss ++ tvs
